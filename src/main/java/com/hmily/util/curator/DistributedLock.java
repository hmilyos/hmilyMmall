package com.hmily.util.curator;
...
public class DistributedLock {
    ...
    // 分布式锁的总节点名
    private static final String ZK_LOCK_PROJECT = "hmily-locks";
    // 分布式锁节点
    ...
    /**
     * @Description: 判断能否获得分布式锁
     */
    public boolean canGetLock() {
        // 使用死循环，当且仅当上一个锁释放并且当前请求获得锁成功后才会跳出
        try {
            client.create()
                    .creatingParentsIfNeeded()
                    ...
                    .forPath("/" + ZK_LOCK_PROJECT + "/" + DISTRIBUTED_LOCK);
            log.info("获得分布式锁成功...");
            return true;                                        // 如果锁的节点能被创建成功，则锁没有被占用
        } catch (Exception e) {
            log.info("获得分布式锁失败...");
            log.error("异常：{}", e);
            return false;
